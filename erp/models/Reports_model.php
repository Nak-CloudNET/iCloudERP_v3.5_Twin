...
class Reports_model extends CI_Model
{
	...
	public function getDepositSuppliers($supplier = NULL){
		$this->db->select("erp_deposits.id as deposit_id, erp_deposits.date as date, erp_deposits.reference, COALESCE(erp_deposits.amount, 0) as total_amount, erp_companies.name as supplier_name, erp_companies.company, erp_companies.phone, erp_companies.supplier_type", false);
        $this->db->from("deposits");
        $this->db->join('erp_companies', 'erp_companies.id = deposits.company_id', 'left');   
		$this->db->where('deposits.reference <>', '');
		if($supplier){
			$this->db->where('erp_deposits.company_id', $supplier);
		}
		$this->db->group_by("erp_deposits.company_id, erp_deposits.reference");
		$q = $this->db->get();
		if($q->num_rows() > 0){
			return $q->result();
		}
	...
	public function getDepositReturnByDepositID($id){
		$this->db->select("SUM(COALESCE(erp_deposits.amount, 0)) as amount_return", false);
        $q = $this->db->get_where('deposits', array('deposit_id' => $id));
		if($q->num_rows() > 0){
			return $q->row();
		}
		return false;
	...
	public function getSupplierBySupType($start_date = NULL, $end_date = NULL, $supplier = NULL, $sup_type){
		$this->db->select("deposits.id as deposit_id, companies.id as sup_id, companies.name as sup_name", false);
		$this->db->from('deposits');
		$this->db->join('companies', 'companies.id = deposits.company_id', 'left');
		$this->db->where(array('companies.supplier_type' => $sup_type, 'companies.group_name' => 'supplier'));
		if($start_date && $end_date){
		   $this->db->where('date_format(erp_deposits.date,"%Y-%m-%d") BETWEEN "' . $start_date . '" and "' . $end_date . '"');
		}
		if($supplier){
		   $this->db->where('deposits.company_id', $supplier);
		}
		$this->db->group_by('deposits.company_id');
        $q = $this->db->get();
		if($q->num_rows() > 0){
			return $q->result();
		}
