...
class Companies_model extends CI_Model
{
	...
	public function returnSupplierDeposit($id, $data, $cdata, $payment)
    {
        if ($this->db->insert('deposits', $data)) {
				$deposit_id = $this->db->insert_id();
				$this->db->update('companies', $cdata, array('id' => $data['company_id']));
    ...
    {
        $q = $this->db->get_where('deposits', array('company_id' => $id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getTotalAmountDepositBySupplier($id)
    {
	...
	public function updateSupplierDeposit($id, $data, $payment)
    {
		if ($this->db->update('deposits', $data, array('id' => $id))) {
			$deposit_items = $this->getTotalAmountDepositBySupplier($data['company_id']);
			$cdata = array(
                'deposit_amount' => $deposit_items->total_amount
            );
			$this->db->update('companies', $cdata, array('id' => $data['company_id']));
