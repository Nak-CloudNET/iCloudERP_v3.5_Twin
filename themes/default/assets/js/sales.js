...
if (slwarehouse = __getItem('slwarehouse')) {
	...
	var old_product_unit;
 	$(document).on("focus", '.product_unit', function () {
 		old_product_unit = $(this).val();
 	}).on("change", '.product_unit', function () {	
		//var alphanumers = /^[a-zA-Z0-9]+$/;
 		var row = $(this).closest('tr');
		/*if (!alphanumers.test($(this).val())) {
 			$(this).val(old_product_unit);			
 			bootbox.alert(lang.check_input);
 			return;
 		}*/
 		var new_product_unit = $(this).val(),
 		item_id = row.attr('data-item-id');
		slitems[item_id].row.product_unit = new_product_unit,
 		__setItem('slitems', JSON.stringify(slitems));		
 		loadItems();
 	});
	...
	if (__getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			var product_id 			= item.row.id,
				...
				oqty				= item.row.oqty,
				product_unit 		= item.row.product_unit ? item.row.product_unit : '';
			...
			tr_html += '<td class="text-right">' +
				...
				'<input class="product_unit form-control text-center" name="product_unit[]" type="text" value="' + product_unit + '" id="product_unit_' + row_no + '"></td>';
