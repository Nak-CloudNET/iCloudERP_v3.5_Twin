...
function loadItems() {
	...
	if (__getItem('positems')) {
        ...
        $.each(positems, function (i, e) {
			...
			if(java_applet == 1) {
				...
				var table_b = '<table style="width: 100%;border-top: 1px solid #000;border-bottom: 1px solid #000;border-collapse: collapse;"><tr class="head" style="background:#2A2725;color:white">';
                    ...
                    table_b +=  '				<th style="text-align: right;border-bottom: 1px solid #000;">Dis</th>';
                    if(total>0){
                        table_b +=  '				<th style="text-align: right;border-bottom: 1px solid #000;">Tax</th>';
					}
				...
				bprTr += '<td style="text-align: right;">'+formatMoney(item_discount)+'</td>';
				if(total>0){
                    bprTr += '<td style="text-align: right;">'+  formatMoney(parseFloat(pr_tax_val) * parseFloat(item_qty))+'</td>';
				}
		...
		if(java_applet == 1) {
			...
			if(order_discount > 0) {
				bill_totals += '<tr><td colspan="5" style="text-align:LEFT;font-weight:bold;">Order Discount</td><td style="text-align:right;font-weight:bold;font-size:18px;"><small>('+posdiscount+'%)</small>$'+ formatMoney(order_discount) +'</td></tr>';
			}
