					...
					if(is_array($rows)){
						...
						?>
							<tr>
								...
								<?php if ($Owner || $Admin || $GP['sales-price']) { ?>
								    <td style="text-align:center; width:100px;"><?= $this->erp->formatMoney($row->unit_price); ?></td>
                                <?php } ?>
								<td style="width: 80px; text-align:center; vertical-align:middle;"><?= $this->erp->formatQuantity($row->quantity); ?></td>
                                <td style="width: 80px; text-align:center; vertical-align:middle;"><?php echo $row->product_unit ?></td>
								<?php
								if ($Settings->tax1) {
							...
							<?php
							$total_unit_cost += $row->sale_unit_cost;
							$r++;
						endforeach;
					}
                    ?>
            ...
            <?php if (!$Supplier || !$Customer) { ?>
                ...
                <div class="buttons">
                    <div class="btn-group btn-group-justified">	
						<div class="btn-group">
                            <a href="<?= site_url('sales/invoiceTaxTwin/' . $inv->id) ?>" class="tip btn btn-primary" title="<?= lang('tax_invoice') ?>">
                                <i class="fa fa-file-text-o"></i>
                                <span class="hidden-sm hidden-xs"><?= lang('tax_invoice') ?></span>
                            </a>
                        </div>
						<div class="btn-group">
                            <a href="<?= site_url('sales/print_invoice_twin/' . $inv->id) ?>" class="tip btn btn-primary" title="<?= lang('invoice') ?>">
                                <i class="fa fa-file-text-o"></i>
