        ...
        var oTable = $('#SLData').dataTable({
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                for (var i = 0; i < aaData.length; i++) {
					status = aaData[aiDisplay[i]][13];
					gtotal += parseFloat(aaData[aiDisplay[i]][14]);
					return_s += parseFloat(aaData[aiDisplay[i]][15]);
					gpaid += parseFloat(aaData[aiDisplay[i]][16]);  
					tdeposit += parseFloat(aaData[aiDisplay[i]][17]);
					dis += parseFloat(aaData[aiDisplay[i]][18]);
					balance += parseFloat(aaData[aiDisplay[i]][19]);
                }
                var nCells = nRow.getElementsByTagName('th');
				nCells[14].innerHTML = currencyFormat(parseFloat(gtotal));
                nCells[15].innerHTML = currencyFormat(parseFloat(return_s));
				nCells[16].innerHTML = currencyFormat(parseFloat(gpaid));
                nCells[17].innerHTML = currencyFormat(parseFloat(tdeposit));
				nCells[18].innerHTML = currencyFormat(parseFloat(dis));
                nCells[19].innerHTML = currencyFormat(parseFloat(balance));
            }
        }).fnSetFilteringDelay().dtFilter([
            {column_number: 1, filter_default_label: "[<?=lang('date');?> (yyyy-mm-dd)]", filter_type: "text", data: []},
            {column_number: 2, filter_default_label: "[<?=lang('sale_no');?>]", filter_type: "text", data: []},
            ...
            {column_number: 7, filter_default_label: "[<?=lang('Containner');?>]", filter_type: "text", data: []},
			{column_number: 8, filter_default_label: "[<?=lang('Bill_No');?>]", filter_type: "text", data: []},
			{column_number: 9, filter_default_label: "[<?=lang('INVOICE');?>]", filter_type: "text", data: []},
			{column_number: 10, filter_default_label: "[<?=lang('qty_vol');?>]", filter_type: "text", data: []},
			{column_number: 11, filter_default_label: "[<?=lang('KGS');?>]", filter_type: "text", data: []},
			{column_number: 12, filter_default_label: "[<?=lang('STATUS');?>]", filter_type: "text", data: []},
			{column_number: 13, filter_default_label: "[<?=lang('sale_status');?>]", filter_type: "text", data: []},
            {column_number: 20, filter_default_label: "[<?=lang('payment_status');?>]", filter_type: "text", data: []},
        ], "footer");
            ...
            <?php if ($Owner || $Admin || $GP['sales-payments'] || $GP['sales-add'] || $GP['sales-export'] || $GP['sales-import'] || $GP['sales-combine_pdf']) { ?>
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
						...
						<li>
                            <a href="#" id="combine" data-action="invoice_pdf">
                                <i class="fa fa-file-pdf-o"></i> <?=lang('combine_invoice_pdf')?>
                            </a>
                        </li>
						<li>
                            <a href="#" id="combine" data-action="tax_invoice_pdf">
                                <i class="fa fa-file-pdf-o"></i> <?=lang('combine_tax_invoice_pdf')?>
                            </a>
                        </li>
                ...
                <div>
                    <table id="SLData" class="table table-bordered table-hover table-striped">
                        ...
                        <tr>
							...
							<th><?php echo $this->lang->line("Containner"); ?></th>
							<th><?php echo $this->lang->line("Bill_No"); ?></th>
							<th><?php echo $this->lang->line("INVOICE"); ?></th>
							<th><?php echo $this->lang->line("qty_vol"); ?></th>
							<th><?php echo $this->lang->line("KGS"); ?></th>
